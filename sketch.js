...
function draw() {
  ...
  */
...

  {
    ...
    //  particle.display();
    if (particles[particleCount].body.position.y > 540) {
      console.log(particles[particleCount].body.position.x)
      //first and last box
      ...
      //second and fifth box
      else if ((particles[particleCount].body.position.x > 85 && particles[particleCount].body.position.x < 167) || (particles[particleCount].body.position.x > 333 && particle.body.position.x < 417)) {
        // checkie++;  
        count++;
        score = score + r2;
        console.log(r2)
...
function mouseReleased() {
  ...
  if (count < 6) {
    if (gameState !== "end") {
      // part = new Particle(mouseX , 30,12);
